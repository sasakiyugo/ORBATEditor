...
function bind(){
  document.querySelectorAll(".view-btn").forEach(b=>b.onclick=()=>switchView(b.dataset.view));$("themeBtn").onclick=()=>{setTheme(isDark()?"light":"dark");renderAll()};$("addRootBtn").onclick=()=>addUnit(null);$("undoBtn").onclick=undo;$("redoBtn").onclick=redo;$("saveBtn").onclick=saveJson;$("openBtn").onclick=()=>$("fileInput").click();$("newBtn").onclick=()=>{if(dirty&&!confirm("未保存の変更を破棄しますか？"))return;state=sampleProject();history=[];future=[];selectedId="u-bde";dirty=false;renderAll()};
