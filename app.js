...
function renderDetail(){
  ...
  $("symbolPreview").innerHTML=unitSymbolSvg(u,{width:250,height:180,labels:true});
...
function frameShape(a,x,y,w,h){if(a==="hostile")return `<polygon class="node-frame" points="${x+w/2},${y} ${x+w},${y+h/2} ${x+w/2},${y+h} ${x},${y+h/2}"/>`;if(a==="neutral")return `<rect class="node-frame" x="${x+w*.12}" y="${y}" width="${w*.76}" height="${h}"/>`;if(a==="unknown")return `<path class="node-frame" d="M${x+w*.5} ${y}Q${x+w*.72} ${y} ${x+w*.72} ${y+h*.24}Q${x+w} ${y+h*.24} ${x+w} ${y+h*.5}Q${x+w} ${y+h*.76} ${x+w*.72} ${y+h*.76}Q${x+w*.72} ${y+h} ${x+w*.5} ${y+h}Q${x+w*.28} ${y+h} ${x+w*.28} ${y+h*.76}Q${x} ${y+h*.76} ${x} ${y+h*.5}Q${x} ${y+h*.24} ${x+w*.28} ${y+h*.24}Q${x+w*.28} ${y} ${x+w*.5} ${y}Z"/>`;return `<rect class="node-frame" x="${x}" y="${y}" width="${w}" height="${h}" rx="2"/>`}
function unitSymbolSvg(u,opt={}){const width=opt.width||180,height=opt.height||130,labels=opt.labels!==false,fw=Math.min(width*.62,125),fh=Math.min(height*.35,58),x=(width-fw)/2,y=labels?height*.27:1;const st=affiliationStyle(u.affiliation),ec=echelon(u.echelonId);return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" aria-label="${esc(u.name)}"><g fill="none" stroke="${st.stroke}" stroke-width="2" color="${st.stroke}" stroke-linecap="round" stroke-linejoin="round">${frameShape(u.affiliation,x,y,fw,fh).replace("/>",` fill="${st.fill}"/>`)}${glyph(u.unitTypeId,x,y,fw,fh)}</g><text x="${width/2}" y="${Math.max(13,y-8)}" text-anchor="middle" fill="${st.stroke}" font-size="${Math.max(10,fh*.27)}" font-weight="700">${esc(ec.symbol)}</text>${labels?`<text x="${width/2}" y="${y+fh+22}" text-anchor="middle" fill="#172233" font-size="13" font-weight="700">${esc(u.name)}</text><text x="${width/2}" y="${y+fh+39}" text-anchor="middle" fill="#667786" font-size="10">${esc(unitType(u.unitTypeId).name)} · ${esc(ec.name)}</text>`:""}</svg>`}
...
function renderChart(){
  const rootId=$("chartRoot").value||selectedId||state.units[0]?.id;if(!rootId){$("chartCanvas").innerHTML="";return}const maxDepth=Number($("chartDepth").value),levels=chartNodes(rootId,maxDepth);const nodeW=170,nodeH=135,gapX=45,gapY=65,maxCount=Math.max(...levels.map(x=>x.length)),width=Math.max(900,maxCount*(nodeW+gapX)+80),height=Math.max(600,levels.length*(nodeH+gapY)+80);const pos=new Map();levels.forEach((list,d)=>{const total=list.length*nodeW+(list.length-1)*gapX,offset=(width-total)/2;list.forEach((u,i)=>pos.set(u.id,{x:offset+i*(nodeW+gapX),y:30+d*(nodeH+gapY)}))});let lines="";pos.forEach((p,id)=>children(id).forEach(c=>{const q=pos.get(c.id);if(q)lines+=`<path d="M${p.x+nodeW/2} ${p.y+nodeH-10}V${p.y+nodeH+gapY/2}H${q.x+nodeW/2}V${q.y+8}"/>`}));const nodes=[...pos].map(([id,p])=>{const u=unit(id),agg=aggregate(id),eq=Object.entries(agg.equipment).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([k,v])=>`${catalogItem(k)?.name||k} ${v}`).join(" / ");return `<g class="org-node" data-id="${id}" transform="translate(${p.x} ${p.y})">${unitSymbolSvg(u,{width:nodeW,height:100,labels:true})}${$("showPersonnel").checked?`<text x="${nodeW/2}" y="115" text-anchor="middle" font-size="10" fill="#526171">人員 ${agg.personnel.toLocaleString()}</text>`:""}${$("showEquipment").checked?`<text x="${nodeW/2}" y="130" text-anchor="middle" font-size="9" fill="#526171">${esc(eq||"装備なし")}</text>`:""}</g>`}).join("");const svg=`<svg id="orgSvg" xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><rect width="100%" height="100%" fill="#fff"/><g fill="none" stroke="#8ea1b3" stroke-width="1.5">${lines}</g>${nodes}</svg>`;$("chartCanvas").className="chart-canvas";$("chartCanvas").innerHTML=svg;applyChartTransform();
}
